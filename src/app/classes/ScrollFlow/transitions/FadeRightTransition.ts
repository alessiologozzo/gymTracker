...
export class FadeRightTransition extends Transition {
    ...
    constructor(durations: number[], timings: string[]) {
        ...
        const inName = `sf-fade-right-in-${durations[0]}-${timings[0]}-${durations[1]}-${timings[1]}`
        const outName = `sf-fade-right-out-${durations[2]}-${timings[2]}-${durations[3]}-${timings[3]}`
